...
Die Kernmodelle befinden sich im Verzeichnis \texttt{src/models/} und umfassen \texttt{transformer.py} für die vollständige Encoder-Decoder Transformer-Implementierung sowie \texttt{language\_model.py} für das GPT-ähnliche Decoder-only Language Model. Die Hilfsfunktionen in \texttt{src/utils/} stellen mit \texttt{tokenizer.py} einen einfachen Tokenizer für Textverarbeitung und mit \texttt{data\_loader.py} Datenlade- und Verarbeitungsfunktionen bereit. Das Training-System in \texttt{src/training/} enthält \texttt{trainer.py} für Training-Loop und Evaluierungsfunktionen sowie \texttt{optimizer.py} für Optimierungsstrategien und Scheduler.
